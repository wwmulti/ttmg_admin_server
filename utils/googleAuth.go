...
func (this *GoogleAuth) GetQrcode(user, secret string) string {
	str := fmt.Sprintf("otpauth://totp/%s?secret=%s", user, secret)
	...
	return fmt.Sprintf("https://api.qrserver.com/v1/create-qr-code/?data=%v&size=%vx%v&ecc=%v", urlencoded, width, height, level)
}
